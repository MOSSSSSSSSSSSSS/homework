...
{
	// 给出输入提示
	std::cout << "现有N×N的棋盘，放入N个皇后，要求所有皇后不在同一行、列、对角线上！" << std::endl << std::endl;
	std::cout << "请输入皇后的个数（6-15）：";
	...
	std::cout << std::endl << "皇后摆法" << std::endl;

	// 输出棋盘情况
	SetQueue(queueProblem, 1);

	// 输出解法总数
	std::cout << "共有" << queueProblem.GetCounter() << "种解法！" << std::endl;
