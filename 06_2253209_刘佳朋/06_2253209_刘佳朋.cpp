...
public:
	...
	void BuildTree()
	{
		...
		if (parent == NULL)
		{
			std::cout << "本家谱内不存在该家庭成员！" << std::endl;
			return;
		}
		...
		{
			std:: cout<< "该成员已经建立过家庭！" << std::endl;
			return;
		}

		// 利用getint函数得到儿女个数，保证健壮性
		std::cout << "请输入" << name << "的儿女个数：";
		...
		while (1)
		{
			...
			{
				std::cout << "姓名为" << name << "的成员已存在！请重新输入！" << std::endl;
				std::cin.clear();
	...
	void AddTreeNode()
	{
		...
		// 得到新添加的节点的姓名
		std::cout << "请输入" << name << "新添加儿子（或女儿）的姓名：";
	...
	void ResetTreeNode()
	{
		...
		while (1)
		{
			...
			if (Search(name) == NULL)
			{
				std::cout << current->name << "已更名为" << name << std::endl;
				current->name = name;
				break;
			}
