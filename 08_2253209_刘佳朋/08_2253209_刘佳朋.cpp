...
public:
	...
	void InitVertex()
	{
		...
		for (int i = 0; i < vertexNumber; ++i)
		{
			...
			{
				// 若已经存在，请重新输入
				std::cout << "顶点" << vertexName << "已存在！请重新输入：";
				i--;
			}
			// 初始化并查集祖先为自己
			else
	...
	void InitEdge()
	{
		...
		std::cout << "请依次输入" << edgeNumber << "条边的起点、终点、权重：" << std::endl;

		for (int i = 0; i < edgeNumber; i++)
		{
			// 设置起点终点
			std::cout << "请输入第" << i + 1 << "条边的信息：";
			std::string src, dst;
			double value;
			std::cin >> src >> dst;
			...
			{
				std::cout << "点信息输入错误，请重新输入：" << std::endl;
				std::cin.clear();
				std::cin.ignore(1024, '\n');                           // 清除缓存区
				i--;
				continue;
	...
	void printMST()
	{
		...
		if (MST == NULL)
		{
			std::cout << "未生成最小生成树，请先生成！" << std::endl << std::endl;
			return;
		}


		// 生成成功
		std::cout << "最小生成树信息如下：" << std::endl;
		// 输出MST中的信息
		for (int i = 0; i < MST->edgeNumber; i++)
			std::cout << "选边信息：起点<" << vertexs[MST->edges[i].src].name << ">  终点<" << vertexs[MST->edges[i].dst].name << ">  权值：" << MST->edges[i].value << std::endl;
...
char Choice()
{
	...
	{
		std::cin >> ret;
		if (std::cin.fail() || ret < 'A' || ret > 'E') // 若输入变量类型不同或数字不在要求范围内
		{
			std::cin.clear();
			std::cin.ignore(1024, '\n');                           // 清除缓存区
...
int main()
{
	...
	while (1)
	{
		...
		case 'B':
			...
			break;
		case 'C':
			if (graph.Kruskal()) std::cout << "Kruskal最小生成树生成完毕！" << std::endl;
			else std::cout << "无法生成最小生成树！";
			break;
		case 'D':
			graph.printMST();
			break;
