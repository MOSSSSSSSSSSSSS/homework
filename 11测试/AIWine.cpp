...
{                                                                                                   //   对方下棋后，轮到ai下。
	Cand best;                                                                                      //   cand表示一个分支，包括这个分支的 新点的位置point，这个分支的value
	long t0, t1, td;                                                                                //   计算时间用的
	start_time = getTime();																		    //   开始时间
	terminateAI = false;																			//   搜索结束的标志，一开始是false
	if (board->chessCount == 0)																		//   如果ai先手，下在中间
	{
		x = board->boardSize / 2;
		y = board->boardSize / 2;
		turnMove(x, y);
	...
	nSearched = 0;																					//   一共搜索的局面数

	bool isSolved = false;
	//先算vcf，连续冲四
	if (board->vcfSearch(&rootBest.point) > 0)														//   引用，最后rootBest（cand）是个下一层的最佳局面，point是那个最佳点
	{
		isSolved = true;
		cout << "MESSAGE VCF success! win point[" << pointX(rootBest.point) - 4 << ", " << pointY(rootBest.point) - 4 << "]" << endl;
	}
	//再算vct，如果没有冲四，再算vct
	if (!isSolved && board->vctSearch(&rootBest.point) > 0)											//   引用，最后rootBest（cand）是个下一层的最佳局面，point是那个最佳点
	{
		isSolved = true;
		cout << "MESSAGE VCT success! win point[" << pointX(rootBest.point) - 4 << ", " << pointY(rootBest.point) - 4 << "]" << endl;
	}
	//算对方的VCT,删除对方能VCT胜的点                                                                     先vcf，再vct，如果没有，就堵对方的vct
	if (!isSolved)
	{
		...
		if (nRootCand == 0)																			//    n rootcand，是根节点分支数
		{
			isSolved = true;
	...
	if (!isSolved)
	{
		...
		for (int depth = MinDepth; depth <= MaxDepth; depth++)                                        //   从mindep深度开始，最多到maxdep深度为止
		{
			t0 = getTime();
			...
			for (int i = 0; i < nRootCand; i++)
			{
				cout << "node：" << pointX(rootCand[i].point) - 4 << ' ' << pointY(rootCand[i].point) - 4 << endl;
			}
			board->sortCand(rootCand, nRootCand);
			for (int i = 0; i < nRootCand; i++)
			{
...
int AIWine::search(int depth, int alpha, int beta, int extend)
{
	...
	if ((q = hashTable->queryRecord(depth, alpha, beta)) != HashTable::InvalidVal) return q;          // 如果是pvs节点
