...
//初始化棋盘
void Board::initBoard(int size)																					//  board是chess的数组，1024大小
{
	...
	//初始化米字范围
	//8个方向
	const int range[8] = { -1,-31,-32,-33,1,31,32,33 };                                                //   指的是  point 变化的点，下面也是
	//8个八卦点
...
//落子
void Board::move(Point p)                                                                  //   有一个新子 ，是 point 落子
{
	assert(check());
	ply++;
	...
	//cout << "之前" << upperLeft << ' ' << lowerRight << endl;
	upperLeft = makePoint(x1, y1);                                                        //    原来分别是棋盘的左上角，右下角
	lowerRight = makePoint(x2, y2);
	//cout << "之后" << upperLeft << ' ' << lowerRight << endl;                             //    之后，是所下点的，左上角的左上角，右下角的右下角
...
Point Board::findVCFStartPoint()
{
	...
	for (int i = chessCount - 2; i >= 0; i -= 2)                             //   棋盘上的每个棋有  x坐标y坐标，有根据x坐标和y得到的point，有根据落子个数对应的编号
...
//排序选点
void Board::sortCand(Cand cand[], int nCand)                                               //   降序 排序,最好的在最前面
{
	Cand key;
	int i, j;
	for (i = 1; i < nCand; i++)
