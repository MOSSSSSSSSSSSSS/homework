...
const int MAX_VCF_DEPTH = 25;			//最大vcf深度
const int MAX_VCT_DEPTH = 25;			//最大vct深度
const int MaxDepth = 5;// 50  6层擦边有点超
...
public:
    ...
    int vcfSearch(int* winVer)
    {
        int startVer, result;
        if ((startVer = findVCFStartVer()) != -1)
        {
            result = vcfSearch(who, 0, startVer, winVer);
            return result;
        }
        return 0;
    }
    int vctSearch(int* winVer)
    {
        int startVer, depth, result;
        if ((startVer = findVCTStartVer()) != -1)
        {
            for (depth = 10; depth <= MAX_VCT_DEPTH; depth += 2)
            {
                result = vctSearch(who, 0, depth, startVer, winVer);
            }
            return result;
        }
        return 0;
    }
    int vctSearch(int searcher, int depth, int maxDepth, int startVer)
    {
        int q;
        //本方能成五
        if (nShape[who][A] >= 1) return 1;
        ...
        {
            move(fiveVer[opp]);
            q = -vctSearch(searcher, depth + 1, maxDepth, startVer);
            undo();
            if (q < 0) q--;
            else if (q > 0) q++;
            return q;
    ...
    int vcfSearch(int searcher, int depth, int startVer, int* winVer)
    {
        int q;
        if (nShape[who][A] >= 1)
        {
            *winVer = findVer(who, A);
            return 1;
        }
        if (nShape[opp][A] >= 2) return -2;
        //对方下一步能成五，挡在成五点
        ...
        {
            move(fiveVer[opp]);
            q = -vcfSearch(searcher, depth + 1, startVer);
            undo();
            if (q < 0) q--;
            else if (q > 0)
            {
    ...
    int vctSearch(int searcher, int depth, int maxDepth, int startVer, int* winVer)
    {
        int q;
        //本方能成五
        if (nShape[who][A] >= 1)
    ...
    int vcfSearch(int searcher, int depth, int startVer)
    {
        int q;
        if (nShape[who][A] >= 1) return 1;
        if (nShape[opp][A] >= 2) return -2;
...
public:
    ...
    {                                                                                                   //   对方下棋后，轮到ai下。
        ...
        terminateAI = false;																			//   搜索结束的标志，一开始是false
        //先算vcf，连续冲四
        board->dep = 0, board->maxdep = 0;
        int lastBest = rootBest.ver;
        memset(isCheckVCT, 0, sizeof(isCheckVCT));
        for (int depth = 1; depth <= MaxDepth; depth++)                                        //   从mindep深度开始，最多到maxdep深度为止
        {

            best = rootSearch(depth, LOSE_SCORE, WIN_SCORE);
            if (best.ver != 0) rootBest = best;

            if (nRootCand > 1) delLoseCand();
            if (rootBest.value == WIN_SCORE || rootBest.value == LOSE_SCORE || nRootCand == 1 || terminateAI) break;

            board->sortCand(rootCand, nRootCand);
            if (depth > 1 && isCheckVCT[rootBest.ver]) continue;
            best.ver = checkOppVct();
            if (nRootCand == 0)
            {
                rootBest.ver = best.ver;
                break;
            }
            else if (nRootCand == 1)
            {
        ...
        x = GetX(rootBest.ver) - 4;
        y = GetY(rootBest.ver) - 4;
        Move(x, y);
    }
    ...
    int search(int depth, int alpha, int beta)
    {
        ...
        if (depth <= 0)
        {
            ...
            if (eval < beta)
            {
                if ((lastVer = board->findVCFStartVer()) != -1 && board->vcfSearch(board->who, 0, lastVer) > 0) return WIN_SCORE;
            }
            return eval;
        }
        ...
        for (int i = 0; i < nCand; i++)
        {
            ...
            if (value >= beta)
            {
                if (!terminateAI && beta != alpha + 1) hashTable->Update(beta, depth, HASH_BETA, cand[i].ver);
                return beta;
            }
            if (value > alpha)
            {
    ...
    void delLoseCand()
    {
        ...
        {
            if (rootCand[i].value == LOSE_SCORE)
            {
    ...
    int delVctLose()
    {
        ...
        for (int i = 0; i < nRootCand; i++)
        {
            Cand& c = rootCand[i];
            board->move(c.ver);
            lastVer = board->findVCTStartVer();

            if (lastVer != -1 && (result = board->vctSearch(board->who, 0, delVctLoseDepth, lastVer)) > 0)//对方VCT胜利
            {
                c.value = LOSE_SCORE;
    ...
    int checkOppVct()
    {
        int lastVer, result, ver = 0, maxLoseSteps = 0;
        ...
        do
        {
            ...
            if (!isCheckVCT[c.ver])
            {
                isCheckVCT[c.ver] = true;
                board->move(c.ver);
                //对手VCT胜利
                if ((lastVer = board->findVCTStartVer()) != -1 && (result = board->vctSearch(board->who, 0, checkOppVctDepth, lastVer)) > 0)
                {
                    c.value = LOSE_SCORE;
                    findVct = true;
                    // 如果都是必败，选择拖延步数最长的为最佳点
