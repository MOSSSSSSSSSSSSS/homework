...
struct tj_time *tj_time_convert(int input_time)
{
	...
	result.tj_second = 0;
	result.tj_minute = 0;
	int second = input_time % 60;
	int min = ((input_time - second)/60)%60;
	int hour = (input_time - second - min * 60)/3600%24;
	int day = (input_time - second - min * 60 - hour)/86400;
	if (hour >= 16)
	{
		day++;
		hour -= 24;
	...
	for (;; year++)
	{
		if ((result.tj_year + year) % 4 == 0 && (result.tj_year + year) % 100 != 0 || (result.tj_year + year) % 400 == 0)
			a = 1;
		else
			a = 0;
		int arr[12] = { 31,28 + a,31,30,31,30,31,31,30,31,30,31 };
		for (int j = 0;j<12; j++)
		{
			if (day >= arr[j])
			{
