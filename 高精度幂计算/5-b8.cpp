...
void calc_num(char pure_decimal[], char out[], char power_table[][N_NUM + 1])
{
	...
	for (int i = 112;i>0; i--)
	{
		...
		else
		{
			...
			for (int j = i;j>0; j--)
			{
				if (power_table[i][j] + out[j - 1] <= 57)
				{
					out[j - 1] = power_table[i][j] + out[j - 1];
				}
				else
				{
					out[j - 1] = power_table[i][j] + out[j - 1]-10;
					out[j-2]++;
				}
			}
			if (x == i)
			{
				for (int j = 113; j > x-1; j--)
				{
					out[j] = 0;
				}
			}
...
void input_pure_decimal(char pure_decimal[])
{
	...
	for (int i = 0; i <= 111; i++)
	{
		...
		{
			if (pure_decimal[i+1] != '0' && pure_decimal[i+1] != '1' && pure_decimal[i+1] != 10 && pure_decimal[i+1] != '\0')
			{
	...
	for (int i = 112;; i--)
	{
		...
		else if (pure_decimal[i] == '0'|| pure_decimal[i] == 10)
		{
			pure_decimal[i] = 0;
		}
		else
...
int main()
{
	...
	*  3、每行的第0列存放幂次，[1]~开始存放十进制的每一位，从而达到高精度存放的目的
